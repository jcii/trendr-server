                ...
                if (!error && response.statusCode == 200) {
                    stockHistorydb.knex.raw(`delete from stock_history`).then(() => {
                        ...
                        for (let i = 0; i < datesArray.length; i++) {
                            databaseArray.push(
                                stockHistorydb.knex.raw(`insert into stock_history values (default, 1, 'netflix', '${data.Elements[0].Symbol}', ${pricesArray[i]}, '${obj.DataPeriod}', ${datesArray[i].unix}, '${datesArray[i].fullDate}', '${datesArray[i].year}', '${datesArray[i].month}', '${datesArray[i].day}')`).then(data => data)
                            )
                        }
